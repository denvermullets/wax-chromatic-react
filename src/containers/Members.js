...
const Members = (props) => {
      ...
      fetch(`${waxUrl}/members/active/${artistId}`, {
        ...
        .then((allMembers) => {
          const currentMembers = allMembers.filter((member) =>
            member.member_artists.some(
              (isActive) => isActive.active_member === true
            )
          )
          ...
          const oldMembers = allMembers.filter((member) =>
            member.member_artists.some(
              (isActive) => isActive.active_member === false
            )
          )
  ...
  return (
    <>
      {activeMembers ? <span>Current: </span> : null}
      <List horizontal>
        {activeMembers
          ? activeMembers.map((member) => (
              <MemberItemActive member={member} key={member.id} />
            ))
          : null}
      </List>
      <br />
      {inActiveMembers ? <span>Previous: </span> : null}
      <List horizontal>
        {inActiveMembers
          ? inActiveMembers.map((member) => (
              <MemberItemInactive member={member} key={member.id} />
            ))
          : null}
      </List>
    </>
