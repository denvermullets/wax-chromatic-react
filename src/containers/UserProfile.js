...
const UserProfile = (props) => {
  ...
  useEffect(() => {
    ...
    if (uri.indexOf("?") > 0) {
      ...
      const waxChromatics = JSON.parse(
        localStorage.getItem("waxChromatics" || "{}")
      )
      fetch(
        `${waxUrl}/oauth/success?oauth_token=${parsed.oauth_token}&oauth_verifier=${parsed.oauth_verifier}&oauth_token_secret=${waxChromatics.oauth_token_secret}&wax=${waxUser.id}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${waxUser.token}`,
  ...
  return (
    <>
      ...
      {discogsAuth ? (
        ...
        <Button content="Connect to Discogs" onClick={() => getTempTokens()} />
      )}
    </>
  )
}
