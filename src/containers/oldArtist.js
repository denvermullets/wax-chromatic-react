...
const ArtistProfile = (props) => {
  ...
  const getArtistReleases = () => {
    ...
    fetch(
      `${url}/artists/${artistId}/releases?sort=year&sort_order=desc&page=1&per_page=100`,
      requestOptions
    )
      .then((response) => response.json())
      .then((artistReleases) => {
        let foundReleases = artistReleases.releases
        foundReleases = foundReleases.filter(
          (singleRelease) =>
            singleRelease.type === "master" && singleRelease.role === "Main"
        )
  ...
  const getVinylAlbums = (releaseId) => {
    ...
    fetch(
      `${url}/masters/${releaseId}/versions?page=1&per_page=100`,
      requestOptions
    )
      .then((response) => response.json())
      .then((releaseInfo) => {
        ...
        findVinyls = findVinyls.filter((vinyl) =>
          vinyl.major_formats.includes("Vinyl")
        )
  ...
  let sortedVinyls = vinyls.sort((a, b) => {
