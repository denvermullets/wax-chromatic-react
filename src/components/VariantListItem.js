...
const VariantListItem = (props) => {
  ...
  let collection = props.collection.filter(
    (collectionAlbum) =>
      parseInt(collectionAlbum.album.d_album_id) ===
      parseInt(props.vinyl.d_album_id)
  )
  let wantlist = props.wantlist.filter(
    (wantlistAlbum) =>
      parseInt(wantlistAlbum.album.d_album_id) ===
      parseInt(props.vinyl.d_album_id)
  )
    ...
    <>
      <Item>
        ...
        <Item.Content>
          ...
          <Item.Meta>{released + " - " + cat_no}</Item.Meta>
          <Item.Description>
            {notes}
            <br />
            <br />
          ...
          <Item.Extra>
            <Label.Group>
              {vinylExtraInfo.styles
                ? vinylExtraInfo.styles.map((style, index) => (
                    <Label color="teal" size={"small"} key={index}>
                      {style}
                    </Label>
                  ))
                : null}
              ...
              <Item.Extra>
                {collection.length > 0 ? (
                  <Button
                    ...
                    color="red"
                    onClick={() =>
                      props.removeVinylFromCollection(collection[0].id)
                    }
                  >
                ...
                {wantlist.length > 0 ? (
                  <Button
                    ...
                    color="red"
                    onClick={() =>
                      props.removeVinylFromWantlist(wantlist[0].id)
                    }
                  >
